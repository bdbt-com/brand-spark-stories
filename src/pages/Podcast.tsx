      ...
      <div className="w-full max-w-2xl">
        {loading || !video ? (
          ...
          <article className="flex flex-col gap-3 sm:gap-5">
            {/* Social links (Instagram, TikTok) above thumbnail */}
            <div className="flex items-center justify-center gap-5 sm:gap-6 pt-1">
              <a
                ...
                className="text-foreground/40 hover:text-foreground/80 transition-colors rounded focus:outline-none focus:ring-2 focus:ring-primary"
              >
            ...
            {gridEpisodes.map((ep) => (
              ...
              >
                <div className="relative aspect-video w-full overflow-hidden rounded-lg sm:rounded-xl border border-border bg-card">
                  <img
                    src={hiResThumb(ep.videoId)}
                    onError={(e) => { const img = e.currentTarget; if (!img.dataset.fallback) { img.dataset.fallback = "1"; img.src = fallbackThumb(ep.videoId); } }}
                    alt={ep.title}
