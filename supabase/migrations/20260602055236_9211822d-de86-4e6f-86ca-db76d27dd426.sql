...
GRANT EXECUTE ON FUNCTION public.get_podcast_click_sessions(timestamp with time zone) TO anon, authenticated, service_role;
...
AS $function$
  ...
  LEFT JOIN (SELECT entered_at::date AS day, COUNT(DISTINCT session_id)::bigint AS cnt FROM page_views WHERE regexp_replace(page_path, '/+$', '') = '/podcast' GROUP BY 1) pc ON pc.day = d.day::date
...
GRANT EXECUTE ON FUNCTION public.get_daily_stats() TO anon, authenticated, service_role;
...
AS $function$
  ...
  LEFT JOIN (SELECT date_trunc('hour', entered_at) AS hour, COUNT(DISTINCT session_id)::bigint AS cnt FROM page_views WHERE entered_at >= date_trunc('day', now() AT TIME ZONE 'UTC') AND regexp_replace(page_path, '/+$', '') = '/podcast' GROUP BY 1) pc ON pc.hour = h.hour
...
GRANT EXECUTE ON FUNCTION public.get_hourly_stats_today() TO anon, authenticated, service_role;
...
AS $function$
  ...
  SELECT
    ...
    (SELECT COUNT(DISTINCT session_id)::bigint FROM page_views, d WHERE entered_at >= d.midnight AND regexp_replace(page_path, '/+$', '') = '/podcast'),
