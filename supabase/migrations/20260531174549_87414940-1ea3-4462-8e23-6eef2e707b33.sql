...
AS $function$
  SELECT
    ...
    COALESCE(p.cnt, 0)::bigint AS podcast_clicks
  ...
  LEFT JOIN (
    ...
    FROM video_clicks WHERE video_id LIKE 'auto-redirect:%' GROUP BY 1
  ) a ON a.day = d.day::date
  LEFT JOIN (
    SELECT entered_at::date AS day, COUNT(*)::bigint AS cnt
    FROM page_views WHERE page_path = '/podcast' GROUP BY 1
